...
/**
 * Chi tiết một phiên "Text to video" - dựng theo đúng 5 bước người dùng đi:
 * Nguồn → Kịch bản đọc → Giọng đọc → Cấu hình video → Dựng video.
 *
 * Mỗi bước là một card đánh số, thanh bước ở header cho biết đang đứng ở đâu.
 * Bố cục, badge, tiến trình job và luồng xóa bám theo trang Auto cut videos để
 * hai tab đọc giống nhau.
 */
 ...
 */
function deriveStage(m: TextToVideoMeta): {
  stage: number;
  active: boolean;
  complete: boolean;
} {
...
/** Thanh 5 bước ở header - cùng ngôn ngữ thị giác với timeline của Videos Project. */
function StageBar({
  stage,
  active,
  complete,
  ...
  return (
    <ol
      ...
      aria-label={tf("ttv.stage-aria", {
        stage,
        label: t(STAGE_LABELS[stage - 1]),
      })}
    >
      {STAGE_LABELS.flatMap((label, i) => {
        const n = i + 1;
        const passed = n < stage || (n === stage && complete);
        const current = n === stage && !complete;
        ...
        actions={
          <Button
            ...
            disabled={
              locked ||
              busy !== null ||
              (source.kind === "url" ? !source.url.trim() : !source.text.trim())
            }
            onClick={() => run("extract")}
          >
            {busy === "extract" ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          {article && (
            <div className="rounded-[var(--radius)] border border-[var(--border)] bg-[var(--bg-subtle)] p-3">
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                ...
                <span className="chip">{tf("ttv.char-count", { n: article.chars })}</span>
              </div>
          ...
          <div>
            ...
            <textarea
              ...
              disabled={locked}
              placeholder={
                source.kind === "url" ? t("ttv.content-placeholder-url") : t("ttv.text-placeholder")
              }
              onChange={(e) => patchSource({ text: e.target.value })}
              onBlur={() => flush()}
            />
            <p className="mt-1 text-xs text-[var(--text-muted)]">
              {t("ttv.content-hint")} · {tf("ttv.char-count", { n: source.text.trim().length })}
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card
        ...
        actions={
          <div className="flex flex-wrap items-end gap-2">
            <label className="flex w-32 flex-col gap-1 text-xs text-[var(--text-muted)]">
            ...
            <Button small disabled={!canScript || busy !== null} onClick={() => run("script")}>
              {busy === "script" ? (
                <Loader2 size={14} strokeWidth={2} className="animate-spin" />
              ) : (
          ...
          <div className="flex flex-col gap-2">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-[var(--text-muted)]">
                      ...
                      <span>
                        {c.durationSec !== null
                          ...
                          : tf("ttv.chunk-est", { time: clock(estimateChunkSeconds(c)) })}
                      </span>
                    </div>
                  </div>
                  <button
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <Button
                ...
                small
                disabled={locked}
                onClick={() => setChunks([...script, { text: "", durationSec: null }], true)}
              >
            ...
            <p className="text-xs text-[var(--text-muted)]">{t("ttv.estimate-warning")}</p>
          </div>
        )}
      </Card>
              ...
              {ASPECTS.map((a) => {
                ...
                return (
                  <button
                    ...
                    disabled={locked}
                    aria-pressed={active}
                    onClick={() => patchOutput({ width: size.width, height: size.height })}
        ...
        <div className="flex flex-col gap-3">
          {running ? (
            ...
            <>
              ...
              <p className="text-xs text-[var(--text-muted)]">{t("ttv.rebuild-disabled")}</p>
            </>
          ) : (
